...
function App() {
  ...
  const updateAnswer = (answer) => {
    setAnswers(oldAnswers => {
      return oldAnswers.map(ans => {
        if(ans.id === answer.id)
          // answer e non ans perchè ans è l'oggetto originale mentre answer è quello modificato (i dati nuovi) [quindi come voteUp ma dati modificati nuovi]
  ...
  {/* ROUTES
    - / => tutte le domande (INDEX ROUTE, la nostra home)
    - /questions/:qid => una domanda "id" con tutte le sue risposte
    ...
    OPZIONE 2:
    - /answers/:aid/edit => modifica risposta (accorciata)
